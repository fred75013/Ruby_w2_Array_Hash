def journalists
    ...
    def compter_handle (tab = journalists)
    puts "Il y a #{tab.count} de handle dans cette array."
    tap = 5
    end
    
    def shortess_handle (tab = journalists)
    puts "Le handle le plus court de cette liste est #{tab.min_by {|nom| nom.size }}"
    end
    
    def handle_5char (tab = journalists)
    puts "Il y a #{tab.count {|nom| nom.size == 6}} handles contenant 5 caractères"
    end
    
    def uppercase_begin (tab = journalists)
    puts "Il y a #{tab.count {|nom| nom =~ /^@[A-Z]/}} handles qui commence par une majuscule après le @"
    end
    
    def alphabetique (tab = journalists)
    puts "Liste de handle par ordre alphabétique : #{tab.sort_by {|nom| nom.downcase}}"
    end
    
    def smalltobig (tab = journalists)
    puts "Liste de handle du plus petit au plus grand : #{tab.sort_by {|nom| nom.length}}"
    end
    
    def position (tab = journalists)
    puts "La position dans l'array de @epenser est #{tab.index('@epenser')}ième"
    end
    
    def repartition (tab = journalists)
...
puts "Quelle question voulez-vous corriger ? (Entrez 1 à 8 please)"
question = gets.chomp.to_i
    ...
    if question == 1
        ...
        puts "Sors-moi une répartition des handle par taille de ces derniers (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, etc)"
        print ">"
        repartition
    else 
        puts "Entrez un numéro de question entre 1 à 8"
    end
end